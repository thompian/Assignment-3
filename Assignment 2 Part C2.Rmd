...
#H - After you calculate the distance, you should look at the values you obtained for maximum latiudinal range. You mainly want samples that have a range greater than 0 as this would indicate that there was a difference in the maximum and minimum range collected for that sample.
...
hist(polyplacophora.filter.geo$Range) #Majority of ranges are from 0-5, lets look at that further
table(polyplacophora.filter.geo$Range) #Looking at counts, a range of 0 is found 27 times, 

polyplacophora.filter.geo <- polyplacophora.filter.geo %>% #Removing ranges with values of 0, this will allow for your samples chosen to have range values of greater than 0, which will be more useful in comparison in your dot tree
  filter(!Range == 0) %>%
...
```
#Statistical analysis - Histogram to see ranges
#Delete samples that have a range of 0, they do not explain aynthing and were only sampled at one latitude, those species would need to be resampled, focus on samples with actual geographical ranges, 
#Bin labels smaller
#Check NAs after filtering
#Set seed when random sampling so you get the same results everytime
#Compare ranges western and eastern 
